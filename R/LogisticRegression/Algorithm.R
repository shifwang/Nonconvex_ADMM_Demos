LqLogReg <- function(A,y,q,
  ...
  #
  #   minimize_{x}     sum_i log(1 + exp(-yi * <ai,x>)  + mu*\|x\|_q^q
  #
  # Args:
  #   A         : matrix [a1,...,ap]
  #   y         : observation vector [y1;...;yn]
  #   q         : 0 < q <= 1 constant
  #   mu        : mu is the regularizer constant
  #   offset    : whether to include the constant term in regression
  ...
  #   beta       : final beta
  #   obj        : vector of obj(x^k) for each k
  ...
  Logistic <- function(x) sum(log(1 + exp(y - A %*% x)))
  Lagrangian <- function(x,z,w) mu*Lq(z) + Logistic(x) + t(w) %*% (x - z) + beta/2 * sum(abs(x - z)^2)
